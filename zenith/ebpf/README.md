# eBPF Process Execution Monitoring - Implementation Guide

## Overview

This directory contains the production-ready eBPF-based kernel monitoring system for Zenith-Sentry. It provides system-wide process execution tracking at the kernel level with minimal overhead.
...
┌──────────────────────┐          ┌──────────────────────┐
│ sys_execve syscall   │          │ ProcessExecveMonitor │
│       ↓              │          │ (Python BCC Manager) │
│ Tracepoint Hook      │──────→   │                      │
│       ↓              │  Ring    │ Event Processing     │
│ BPF Program          │ Buffer   │ Threat Detection     │
│ (Lock-Free)          │          │ JSON Output          │
└──────────────────────┘          └──────────────────────┘
```
...
| File | Purpose |
|------|---------|
| `execve_monitor.c` | eBPF kernel program (C) - attached to syscalls:sys_enter_execve tracepoint |
| `process_execve_monitor.py` | User-space manager script - loads eBPF, handles events, threat detection |
| `install_ebpf_deps.sh` | Automated setup script for BCC toolkit and dependencies |
...
Each execve event produces a JSON object:
