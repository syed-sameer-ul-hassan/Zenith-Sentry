...
setup_ebpf() {
    ...
    if [[ -f /etc/os-release ]]; then
        ...
        case "$ID" in
            ubuntu|debian)
                apt-get update -qq 2>/dev/null && apt-get install -y bpf-tools libbpf-dev linux-headers-$(uname -r) python3-dev 2>&1 | grep -v "^Get:" || true
                ;;
            fedora|rhel|centos)
                dnf install -y bcc-tools libbpf-devel kernel-devel python3-devel 2>&1 | grep -v "^" || true
                ;;
